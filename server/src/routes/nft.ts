...
function detectIcon(role: string, skills: string[]): IconType {
...
function generateSvg(agent: Agent): string {
  const accent  = nameToAccent(agent.name);
  const icon    = buildIcon(detectIcon(agent.role, agent.skills), accent);
  const { svg: pills, bottomY } = buildSkillPills(agent.skills, accent);

  const divY    = bottomY + 30;
  const footY   = divY + 22;
  const height  = footY + 30;

  const regDate = new Date(agent.registeredAt).toLocaleDateString('en-US', {
  ...
  <text x="380" y="30" font-family="'Courier New',monospace" font-size="10"
        fill="#404040" text-anchor="end" letter-spacing="1">AGID #${agent.identityNftSerial}</text>
  ...
  <text x="200" y="306" font-family="'Inter','Helvetica Neue',Arial,sans-serif"
        ...
        text-anchor="middle" letter-spacing="-0.5">${esc(agent.name)}</text>
  <text x="200" y="330" font-family="'Courier New',monospace" font-size="11"
        fill="#505050" text-anchor="middle" letter-spacing="2.5">${esc(agent.role.toUpperCase())}</text>
